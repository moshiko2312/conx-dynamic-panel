...
- **`warm_white` / `warm_yellow` fail in ConX and in Z2M:** documented Z2M converter bug — exposes use underscores (`warm_white`) while the Tuya lookup map uses `warmwhite` / `warmyellow`. ConX no longer offers these in the card and never writes them to the device; drafts remapped to `white` / `yellow`. Use `white` / `yellow` until Z2M is fixed, or apply an external converter that aligns the keys.
- **`blue` looks cyan:** Z2M option id `blue` drives a cyan LED on these panels (no true deep blue). ConX keeps sending `blue` to HA/Z2M; the card preview swatch and labels describe the real appearance. `cyan` is still a separate enum value.
