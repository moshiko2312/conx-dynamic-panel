...
const EN: Record<string, string> = {
  ...
  "card.delete_blocked_scheduler":
    "Cannot delete this profile — used by scheduler task(s): {tasks}. Remove or retarget those tasks first.",
  ...
  "info.operate_body":
    "Shows only the faceplate for day-to-day use. Open the title-row menu and choose Settings to return to the full editor.",
  ...
  "scheduler.hint":
    "Each trigger activates a profile at its time and stays active until the next trigger fires — in this task, another task, or after wrapping past midnight. Before the first trigger, the default profile is used. Panel holiday pauses this panel’s schedulers; master holiday pauses every panel.",
  ...
  "scheduler.conflict_body":
    "Two enabled triggers would activate different profiles at the same time. Change the time, days, months, or profiles, then save again.",
  "scheduler.conflict_ok": "OK",
  "scheduler.trigger_hint":
    "A trigger stays active until the next one fires — including overnight, into the next day, until something else supersedes it.",
...
const HE: Record<string, string> = {
  ...
  "card.delete_blocked_scheduler":
    "לא ניתן למחוק את הפרופיל — בשימוש במשימות תזמון: {tasks}. הסירו או שנו את המשימות קודם.",
  ...
  "info.operate_body":
    "מציג רק את תצוגת הפאנל לשימוש יומיומי. בתפריט בשורת הכותרת בחרו «הגדרות» כדי לחזור לעורך המלא.",
  ...
  "scheduler.hint":
    "כל טריגר מפעיל פרופיל בשעתו ונשאר פעיל עד שהטריגר הבא נכנס — באותה משימה, במשימה אחרת, או אחרי חצות. לפני הטריגר הראשון נבחר פרופיל ברירת המחדל. מצב חג בפאנל עוצר את התזמון כאן; מצב חג ראשי עוצר בכל הפאנלים.",
  ...
  "scheduler.conflict_body":
    "שני טריגרים פעילים יפעילו פרופילים שונים באותה שעה. שנו את השעה, הימים, החודשים או הפרופילים ושמרו שוב.",
  "scheduler.conflict_ok": "אישור",
  "scheduler.trigger_hint":
    "טריגר נשאר פעיל עד שהבא אחריו נכנס — כולל מעבר חצות, אל תוך היום הבא, עד שמשהו אחר תופס את מקומו.",
...
const RU: Record<string, string> = {
  ...
  "card.delete_blocked_scheduler":
    "Нельзя удалить профиль — он используется в задачах планировщика: {tasks}. Сначала удалите или измените эти задачи.",
  ...
  "info.operate_body":
    "Показывает только лицевую панель для повседневного использования. В меню строки заголовка выберите «Настройки», чтобы вернуться к полному редактору.",
  ...
  "scheduler.hint":
    "Каждый триггер активирует профиль в своё время и остаётся активным до следующего триггера — в этой задаче, в другой задаче или после полуночи. До первого триггера используется профиль по умолчанию. Праздник панели останавливает планировщик здесь; мастер-праздник — на всех панелях.",
  ...
  "scheduler.conflict_body":
    "Два включённых триггера активировали бы разные профили в одно и то же время. Измените время, дни, месяцы или профили и сохраните снова.",
  "scheduler.conflict_ok": "OK",
  "scheduler.trigger_hint":
    "Триггер остаётся активным до следующего — включая переход через полночь, до тех пор пока его не сменит другой.",
