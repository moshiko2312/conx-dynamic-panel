...
- **A user guide now ships with the repository:** `docs/user-guide/index.html` is a self-contained page in Hebrew, English and Russian (RTL for Hebrew), with a sidebar, scroll spy and a language choice kept in `localStorage`. It covers the idea behind the panel, installation, every control on the card, the draft / applied-snapshot / hardware model, the services and events, and a symptom table. Eight screenshots are embedded as data URIs, so the file opens with no assets beside it.
